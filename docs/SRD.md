...
**日期：** 2026-02-24
**状态：** 已批准
**更新：** 修复坐标系统极点奇异性、四元数旋转万向锁、赤经线渲染问题
...
| FR-3.2 | 显示所有行星在天空中的位置（高度角 > -20° 的行星均显示，地平线以下半透明） | 高 |
| FR-3.3 | 显示背景恒星（视星等 < 6.5，约9000颗），按星等设置亮度和大小 | 高 |
| FR-3.4 | 显示黄道线（行星运行轨迹带）和天赤道线 | 中 |
| FR-3.5 | 显示赤经赤纬网格（赤纬圈5条，赤经线12条），采用分段渲染避免视锥体裁剪问题 | 中 |
| FR-3.6 | 显示88个星座的连线和中英文名称，支持鼠标悬停高亮 | 中 |
| FR-3.7 | 支持拖拽旋转视角（桌面：鼠标拖拽；移动端：单指滑动），使用轨迹球算法避免万向锁 | 高 |
| FR-3.8 | 支持惯性旋转：松手后视角继续旋转并逐渐减速 | 中 |
| FR-3.9 | 显示方位角标注（东南西北） | 中 |
| FR-3.10 | 点击行星/恒星显示详细信息 | 中 |
| FR-3.11 | 极点奇异性处理：天顶/天底附近方位角计算采用时角公式，确保赤经线均匀分布 | 高 |

### FR-4 数据与精度

| 编号 | 需求描述 | 优先级 |
|------|---------|--------|
| FR-4.1 | 行星位置精度：优于1角分（使用 astronomy-engine 库，基于 VSOP87 算法） | 高 |
| FR-4.2 | 坐标转换精度：赤道坐标 → 地平坐标转换采用标准球面三角学公式 | 高 |
| FR-4.3 | 极点奇异性处理：天顶/天底附近（`cos(Alt) < 1e-10`）采用时角公式计算方位角 | 高 |
| FR-4.4 | 支持时间范围：公元前3000年至公元3000年 | 中 |
| FR-4.5 | 全部客户端计算，无需外部API，支持离线使用 | 高 |
| FR-4.6 | 考虑大气折射修正（地平线附近天体） | 低 |
...
- 移动端帧率 ≥ 24fps
- 赤经线分段渲染优化：每条赤经线分为南北两段，避免视锥体裁剪问题
- 四元数旋转优化：使用轨迹球算法，避免万向锁，提供平滑旋转体验
...
### NFR-5 数学准确性
- 坐标转换采用标准天文学公式（球面三角学）
- 极点奇异性处理：北天极 `azimuth = 180° - HA`，南天极 `azimuth = HA`
- 本地恒星时计算采用 IAU 标准公式
- 所有角度计算考虑 360° 环绕归一化
...
4. 天空视图能正确显���行星高度角和方位角
5. 移动端触摸操作流畅，无明显卡顿
6. 赤经线在极点附近均匀分布，无重叠或断裂
7. 鼠标拖拽旋转平滑自然，无万向锁或偏斜现象
8. 赤经赤纬网格在任意视角下完整渲染，无视锥体裁剪问题
9. 星座悬停高亮响应灵敏，名称显示准确
10. 惯性旋转效果自然，减速平滑
...
### 6.1 坐标系统
- **地平坐标系**：X=东，Y=上（天顶），Z=北（-Z=南）
- **赤道坐标系**：RA（赤经）/Dec（赤纬），标准天文坐标
- **Three.js 世界坐标系**：与地平坐标系一致，相机位于原点

### 6.2 关键算法
- **赤道 → 地平转换**：球面三角学公式，时角 HA = LST - RA
- **极点奇异性处理**：`cos(Alt) < 1e-10` 时使用时角公式
- **轨迹球旋转**：旋转轴垂直于鼠标移动方向，四元数实现
- **赤经线分段**：每条线分为南半球段（-90° 到 0°）和北半球段（0° 到 +90°）

### 6.3 性能优化
- BufferGeometry 替代 Geometry
- 星点使用 THREE.Points（单次 draw call）
- 移动端星星数量降级（mag < 5.0）
- 赤经线分段渲染优化视锥体裁剪
